...
- [Быстрый старт](#быстрый-старт)
  - [Сценарий первого запуска (5-10 минут)](#сценарий-первого-запуска-5-10-минут)
...
4) Импортируйте flow в NiFi из файлов:
   ...
   - `nifi-templates/SampleKafka2Postgres.json`
   Подробный гайд: `MENTEE_GUIDE_JSON_FLOWS.md`.
...
## Адреса и доступы
- NiFi: http://localhost:18443/nifi/
  > 💡 **Примечание:** В текущей конфигурации NiFi доступен по HTTP, логин не требуется. Если вы включите HTTPS/аутентификацию, креды заданы в `docker-compose.yml` (`admin` / `Password123456`).
...
3. Попробуйте полный сброс: `docker compose down -v && docker compose up -d`
   ...
   После этого потребуется заново выполнить `/nifi-templates/SampleKafka2Postgres.sql` и импортировать flow.
